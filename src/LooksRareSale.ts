...
export function handleTakerAsk(event: TakerAsk): void {
  ...
  if(!dailyCollectionSnapshotEntity) {
    let dailyCollectionSnapshotEntity = new dailyCollectionSnapshot(event.params.collection.toString())
  ...
  dailyCollectionSnapshotEntity.dailyTransactions = collectionEntity.dailyTransactions + 1
